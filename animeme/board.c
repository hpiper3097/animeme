...
int CheckBoard(const S_BOARD* pos) {
	...
	for(sq64 = 0; sq64 < 64; ++sq64) {
		...
		if(PieceMin[t_piece] == TRUE) {
			//printf("%d is the color %d.  at square %d there is a %d. \n", t_piece, color, sq64, pos->pieces[sq120]);
			t_minPce[color]++;
		}
		t_material[color] += PieceVal[t_piece];
	}
		//printf("MinPce:%d == T_MinPce:%d\n", pos->minPce[BLACK], t_minPce[BLACK]);
		ASSERT(pos->minPce[WHITE] == t_minPce[WHITE] && pos->minPce[BLACK] == t_minPce[BLACK]);
...
void ResetBoard(S_BOARD *pos) { //self explanatory
