...
int ThreeFoldRep(const S_BOARD* pos) {
	...
	for(i = 0; i < pos->hisPly; ++i) {
		if(pos->history[i].hashKey == pos->posKey) {
			r++;
		}
	}
	return r;
...
int checkResult(S_BOARD* pos) {
	...
	if(InCheck == TRUE) {
		if(pos->side == WHITE) {
			printf("0-1 {black mates white (claimed by %s)}\n",NAME); return TRUE;
		} else {
			printf("0-1 {white mates black (claimed by %s)}\n",NAME); return TRUE;
		}
	} else {
